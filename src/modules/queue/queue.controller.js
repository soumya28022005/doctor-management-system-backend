...
export const getQueueStatus = asyncHandler(async (req, res) => {
  const { doctorId, clinicId, date, scheduleId } = req.params;
  const queue = await queueService.getQueueStatus(doctorId, clinicId, date, scheduleId);
  res.status(200).json(new ApiResponse(true, "Queue status fetched", { queue }));
});

export const next = asyncHandler(async (req, res) => {
  const { doctorId, clinicId, date, scheduleId } = req.params;
  const result = await queueService.nextToken(req.user, doctorId, clinicId, date, scheduleId);
  res.status(200).json(new ApiResponse(true, "Moved to next token", result));
});

export const previous = asyncHandler(async (req, res) => {
  const { doctorId, clinicId, date, scheduleId } = req.params;
  const result = await queueService.previousToken(req.user, doctorId, clinicId, date, scheduleId);
  res.status(200).json(new ApiResponse(true, "Moved to previous token", result));
});

export const skip = asyncHandler(async (req, res) => {
  const { doctorId, clinicId, date, scheduleId } = req.params;
  const result = await queueService.skipToken(req.user, doctorId, clinicId, date, scheduleId);
...
export const recall = asyncHandler(async (req, res) => {
  ...
  const result = await queueService.recallToken(req.user, doctorId, clinicId, date, scheduleId, token);
  res.status(200).json(new ApiResponse(true, "Token recalled", result));
});

export const pause = asyncHandler(async (req, res) => {
  const { doctorId, clinicId, date, scheduleId } = req.params;
  const result = await queueService.pauseQueue(req.user, doctorId, clinicId, date, scheduleId);
  res.status(200).json(new ApiResponse(true, "Queue paused", result));
});

export const resume = asyncHandler(async (req, res) => {
  const { doctorId, clinicId, date, scheduleId } = req.params;
  const result = await queueService.resumeQueue(req.user, doctorId, clinicId, date, scheduleId);
  res.status(200).json(new ApiResponse(true, "Queue resumed", result));
});

export const close = asyncHandler(async (req, res) => {
  const { doctorId, clinicId, date, scheduleId } = req.params;
  const result = await queueService.closeQueue(req.user, doctorId, clinicId, date, scheduleId);
  res.status(200).json(new ApiResponse(true, "Queue closed", result));
});

export const reopen = asyncHandler(async (req, res) => {
  const { doctorId, clinicId, date, scheduleId } = req.params;
  const result = await queueService.reopenQueue(req.user, doctorId, clinicId, date, scheduleId);
...
export const emergency = asyncHandler(async (req, res) => {
  ...
  const appointment = await queueService.emergencyToken(req.user, doctorId, clinicId, date, scheduleId, patientId);
